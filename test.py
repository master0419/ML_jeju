...
CONFIG = {
    ...
    "lag_periods": [1, 2, 3, 6, 12],  # 래그 변수 기간 (window 외 추가)
    ...
    # ---- 실험 보고서 설정 ----
    "experiment_name": "",           # 실험명 (비어있으면 기본명)
    "experiment_changes": [],         # 변경 사항 리스트
}
...
def create_features(monthly, config=None):
    """슬라이딩 윈도우 + 시계열 피처를 생성합니다.

    직전 window_size개월의 값을 피처로 사용 (val_t-1 ~ val_t-W).
    추가로 달력·이동통계·변화율 피처도 함께 생성합니다.
    """
    if config is None:
        config = CONFIG
    target = config["target_col"]
    ...
    # (3) 윈도우 기반 이동통계
    df[f'rolling_mean_{window_size}'] = df[target].rolling(window_size).mean()
    df[f'rolling_std_{window_size}'] = df[target].rolling(window_size).std()
    if window_size >= 3:
    ...
    # NaN 제거 (윈도우 warmup에 의한 초기 행)
    df_model = df.dropna(subset=feature_cols).copy()

    print(f"  ✓ 슬라이딩 윈도우: {window_size}개월")
...
def recursive_forecast(model, monthly, feature_cols, config=None):
    ...
    슬라이딩 윈도우 방식: 직전 W개월 값을 피처로 사용하여 재귀 예측.
    """
    if config is None:
        config = CONFIG
    target = config["target_col"]
    ...
    for tgt_date in future_dates:
        ...
        if len(recent) >= window_size:
            row[f'rolling_mean_{window_size}'] = np.mean(recent[-window_size:])
            row[f'rolling_std_{window_size}'] = np.std(recent[-window_size:])
        else:
            row[f'rolling_mean_{window_size}'] = np.nan
            row[f'rolling_std_{window_size}'] = np.nan

        if window_size >= 3 and len(recent) >= 3:
            row['rolling_mean_3'] = np.mean(recent[-3:])
            row['rolling_std_3'] = np.std(recent[-3:])
        else:
            row['rolling_mean_3'] = np.nan
            row['rolling_std_3'] = np.nan
...
def main(config=None):
    ...
    if report_path:
        print(f"  📎 분석보고서 링크: {os.path.abspath(report_path)}")
